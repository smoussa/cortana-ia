...
public class FastOptimizer implements Solver {
	...
	private static final Logger log = Logger.getLogger(FastOptimizer.class
			.getName());
	...
	/*
	 * int preferences[][] = new int[][] { {2,2,55,193,180,111},
	 * {2,2,110,68,193,148}, {0,1,112,67,149,177}, {1,1,69,87,142,189},
	 * {1,1,87,74,72,167}, {1,2,72,77,78,37}, {1,2,67,78,154,67},
	 * {0,2,140,141,3,23} };
	 */

	int preferences[][] = new int[][] { { 1, 2, 72, 77, 78, 37 },
			{ 2, 2, 55, 193, 180, 111 }, { 0, 1, 112, 67, 149, 177 },
			{ 1, 1, 87, 74, 72, 167 }, { 2, 2, 110, 68, 193, 148 },
			{ 1, 1, 69, 87, 142, 189 }, { 1, 2, 67, 78, 154, 67 },
			{ 0, 2, 140, 141, 3, 23 } };

	int own[][] = new int[][] { { 2, 4, 2, 0 }, { 0, 3, 5, 0 }, { 7, 3, 2, 0 },
			{ 9, 3, 3, 0 }, { 1, 1, 2, 2 }, { 1, 0, 0, 4 }, { 1, 1, 4, 0 } };

	// Client preferences (fligth, hotel g/c & entertainment 1, 2, 3)
	/*
	 * int preferences[][] = new int[][] { { 1, 1, 94, 75, 174, 161}, { 3, 3,
	 * 68, 174, 13, 43}, { 2, 3, 122, 175, 189, 187}, { 2, 3, 126, 77, 181, 79},
	 * { 2, 2, 138, 135, 198, 12}, { 0, 3, 109, 13, 48, 17}, { 1, 3, 92, 84,
	 * 147, 103}, { 2, 3, 148, 94, 10, 1} };
	 */
	int bestInFlight[][] = new int[][] { { 0, 1, 2, 3 }, { 1, 0, 2, 3 },
			{ 2, 1, 3, 0 }, { 3, 2, 1, 0 } };

	int bestOutFlight[][] = new int[][] { { 0, 1, 2, 3 }, { 1, 2, 0, 3 },
			{ 2, 3, 1, 0 }, { 3, 2, 1, 0 } };

	// What we own
	/*
	 * int own[][] = new int[][] { { 1, 2, 4, 1}, { 1, 2, 1, 4}, { 1, 1, 4, 5},
	 * { 0, 1, 0, 3}, { 0, 0, 0, 1}, { 0, 1, 1, 1}, { 3, 0, 1, 2}};
	 */
	...
	int calcBestScore() {
		...
		for (int type = 0; type < 3; type++) {
			...
			// Copy the clients preferences value for a specific entertainment
			// type
	...
	void bnb(int hscore, long stay, long alloc, int allocPos, long stuff1,
			long stuff2, int flights) {
		...
		if (allocPos >= MAX_ALLOC_POS) {
			...
			for (int i = 0; i < 8; i++) {
				...
				if (a != 0) {
					...
					prefs = preferences[i];
					score += 1000 - ((((tmp1 = inf - 1 - prefs[0]) >= 0) ? tmp1
							: -tmp1) + (((tmp2 = outf - 2 - prefs[1]) >= 0) ? tmp2
							: -tmp2)) * 100;
					...
					if (inScore > bound) {
						for (int j = 0; j < 4; j++) {
							...
							if (outF > 0 && outday >= inday) {
								...
								if (outScore > bound) {
									long bits = 0L;
									for (int b = 1 << outday, min = 1 << inday; b >= min; b = (b >> 1)) {
										bits |= b;
									}
									...
									outflight: for (int h = 0; h < 2; h++) {
										long newStuff1 = outStuff1;
										int hotelPos = (h == 0) ? GOOD_POS
												: CHEAP_POS;
										for (int d = inday; d <= outday; d++) {
											if (((stuff1 >> (hotelPos + d * 4)) & 0xf) == 0) {
												// No hotel => can not stay this
												// nights so we should
												// try next hotel type in this
												// allocation.
												continue outflight;
											} else {
										...
										if (newScore > bound) {
											// Inling the entertainment stuff
											// here!!!
											bnb(newScore, newStay, newAlloc,
													allocPos + 1, newStuff1,
													stuff2, newFlights);
										}
									}
								}
			...
			// }
			if (newScore > bound) {
				bnb(newScore, stay, alloc, allocPos + 1, stuff1, stuff2,
						flights);
			}
			// }
			...
			if (a == 0) {
				...
				// System.out.println("client " + (user + 1) + " in=" + (inf +
				// 1)
				...
				for (int e1d = start; e1d <= stop; e1d++) {
					...
					if ((e1d == stop) || (((stuff2 >> (e1d * 4)) & 0xf) != 0)) {
						if (e1d < stop) {
							...
							// No Ent - I am high - remove me and add highest of
							// others
								...
								if (((ent != 0) && (e1d != e2d))
									...
									if (e2d < stop) {
										if (prefs[4] < enterMin[1]) {
											// Take one from a high! - high has
											// lowered already...
											// but still maybe not enterMax?
											e2score = e1score - enterMax[1]
													+ prefs[4];
										} else {
											// I am high!
											e2score = e1score;
										}
									} else if (enterMin[1] >= 0
											&& prefs[4] >= enterMin[1]) {
										// No Ent - I am high - remove me and
										// add highest of others
	...
	public int solve() {
		...
		sb.append(EOL).append("-------------------------------------------")
				.append(EOL).append("Final score").append(EOL)
				.append("-------------------------------------------")
				.append(EOL);
		...
		sb.append("-------------------------------------------").append(EOL)
				...
				.append("Bnbs: ").append(bnbs).append("    \tBound: ")
				.append(bound).append(EOL)
				.append("-------------------------------------------");
	...
	private void setLatestAlloc(StringBuffer sb, long stay, long alloc) {
		...
		for (int i = 0; i < 8; i++) {
			...
			if (inf != -1) {
				...
				sb.append("Client ").append(i + 1)
						.append(" does not go, score = 0");
	...
	public static void main(String[] args) {
		FastOptimizer f = new FastOptimizer();
		//f.showDebug();
		f.solve();
		int[][] i = f.getLatestAllocation();
