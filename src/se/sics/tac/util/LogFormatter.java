...
public class LogFormatter extends Formatter {
	...
	public synchronized String format(LogRecord record) {
		...
		sb.append(dFormat.format(date)).append(' ').append(record.getLevel())
				.append(' ').append(getAliasFor(record.getLoggerName()))
				.append('|').append(record.getMessage()).append(EOL);
	 ...
	 * 
	 * @param aliasMap
	 *            in the form name1, alias1, name2, alias2, etc
		...
		for (int i = 0, n = childs.length; i < n; i++) {
			if (childs[i].isFile() && matchName(childs[i].getName(), base)) {
				if (forceRemove) {
					...
					// Lock detected
					do {
						System.err.print("Remove log lock file '"
								+ childs[i].getName() + "' (Y/N)? ");
						c = readToEOL();
						if (c == 'Y' || c == 'y') {
							notDone = false;
