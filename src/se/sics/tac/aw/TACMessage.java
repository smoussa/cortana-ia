...
public class TACMessage {
	...
	private static final Logger log = Logger.getLogger(TACMessage.class
			.getName());
	...
	void setReceivedMessage(String receivedMessage) {
		...
		if ("getQuote".equals(type)) {
			...
			if (data instanceof Bid) {
				...
				log.finest("submitting bid (" + type + ") to auction "
						+ auction + " ("
						+ TACAgent.getAuctionTypeAsString(auction) + ')');
			}
		}
		...
		log.finest("XML in: '" + receivedMessage + "' responseTime: "
				+ getResponseTime() + " avg: " + getAverageResponseTime()
		...
		if (nextTag() && (!isDeclaration() || nextTag())) {
			if (isTag(type)) {
				...
				throw new IllegalStateException("Message not expected: " + type
						+ " -> " + getTag());
			}
		} else {
