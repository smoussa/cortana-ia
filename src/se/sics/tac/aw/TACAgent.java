...
public class TACAgent implements Task, TACMessageReceiver {
	...
	private static final Logger log = Logger
			.getLogger(TACAgent.class.getName());
	...
	private final static String[] statusName = { "no error", "internal error",
			...
			"cannot withdraw bid", "bad bidstring format", "not supported",
			"game type not supported" };

	private final static int[] statusCodes = { NO_ERROR, INTERNAL_ERROR,
			AGENT_NOT_AUTH, GAME_NOT_FOUND, NOT_MEMBER_OF_GAME, GAME_FUTURE,
			GAME_COMPLETE, AUCTION_NOT_FOUND, AUCTION_CLOSED, BID_NOT_FOUND,
			TRANS_NOT_FOUND, CANNOT_WITHDRAW_BID, BAD_BIDSTRING_FORMAT,
			NOT_SUPPORTED, GAME_TYPE_NOT_SUPPORTED };
	...
	private final static String[] auctionType = new String[] { "Inflight 1",
			"Inflight 2", "Inflight 3", "Inflight 4", "Outflight 2",
			"Outflight 3", "Outflight 4", "Outflight 5", "Cheap Hotel 1",
			"Cheap Hotel 2", "Cheap Hotel 3", "Cheap Hotel 4", "Good Hotel 1",
			"Good Hotel 2", "Good Hotel 3", "Good Hotel 4", "Alligator 1",
			"Alligator 2", "Alligator 3", "Alligator 4", "Amusement 1",
			"Amusement 2", "Amusement 3", "Amusement 4", "Museum 1",
...
//	private boolean isNextGameTaskRunning = false;
	 ...
	 * @deprecated
	 */
	public TACAgent(AgentImpl agent, String host, int port, String user,
			String pwd, String className) {
		this(agent);
		if (host != null) {
			this.host = host;
			this.port = port;
	...
	public TACAgent(AgentImpl agent, ArgEnumerator a, Properties config) {
		...
		userName = trim(a.getArgument("-agent",
				config.getProperty("agent", null)));
		...
		host = trim(a.getArgument("-host",
				config.getProperty("host", "localhost")));
		port = a.getArgument("-port", getInt(config, "port", 6500));
		connectionClassName = trim(a.getArgument("-connection",
				config.getProperty("connection", "se.sics.tac.aw.TACReader")));

		gameType = trim(a.getArgument("-gameType",
				config.getProperty("gameType", null)));
		...
		int consoleLevel = a.getArgument("-consoleLogLevel",
				getInt(config, "consoleLogLevel", 3));
		int fileLevel = a.getArgument("-fileLogLevel",
				getInt(config, "fileLogLevel", 0));
		this.logPrefix = trim(a.getArgument("-logPrefix",
				config.getProperty("logPrefix", "aw")));
		...
		do {
			try {
				...
				log.log(Level.SEVERE,
						"could not create TACConnection object of class "
	...
	public static String auctionCategoryToString(int category) {
		return (category >= CAT_FLIGHT) && (category < categoryName.length) ? categoryName[category]
...
//		int auction = bid.getAuction();
	...
	public void replaceBid(Bid oldBid, Bid bid) {
		...
		if (auction != oldAuction) {
			throw new IllegalArgumentException(
					"Bids do not have same AuctionID");
		}
		bid.submitted();
	...
	private void printOwn(String type, int startAuction, int endAuction,
		...
		for (int i = startAuction; i < endAuction; i++) {
			...
			sb.append("(").append(getAllocation(i)).append('-')
					.append(getOwn(i)).append('|').append(getProbablyOwn(i))
		...
		if (connection != null) {
			try {
				...
				log.log(Level.WARNING,
						"could not send message " + msg.getType(), e);
				reset(0, connection);
			}
		} else {
	...
	public void messageReceived(TACMessage msg) {
		if (msg.isTACError()) {
			...
			if ("getGameConsts".equals(type)) {
				...
				log.warning("ignoring error for getGameConst: "
						+ msg.getValue());
			} else {
			...
			if (msg.isTag("auth")) {
				...
				if (display != null) {
					String status = playingGameType != null ? userName
							+ ": Showing game " + nextGameID + " of type "
							+ playingGameType : userName + ": Showing game "
							+ nextGameID;
	...
	private void requestQuote(Quote quote, TACConnection conn, boolean force) {
		...
		if (auctionID > 0) {
			...
			if (!force && (pendingQuotes[auction] > 0)
				...
				// Quote is already pending and it has not passed sufficient
				// time
				...
				try {
					...
					if (bid != null) {
						...
						if ((id = bid.getID()) != Bid.NO_ID) {
							...
							// Request HQW for previous bid if it currently is
							// being
					...
					log.log(Level.SEVERE,
							"could not request quote for auction " + auction
									+ " (" + getAuctionTypeAsString(auction)
									+ ')', e);
	...
	private synchronized void revertBid(Bid bid, int status) {
		...
		if (bid.same(activeBid)) {
			...
			Bid child;
			while ((child = activeBid.getReplacing()) != null
					&& !child.same(bid)) {
				activeBid = child;
			}
		...
		if (bid != null) {
			if (status == NO_ERROR) {
				try {
					...
					log.log(Level.SEVERE, "agent could not handle bidRejected",
							e);
				}
			} else {
				try {
	...
	private void handleTransInfo(TACMessage msg) {
		...
		while (msg.nextTag()) {
			if (msg.isTag("/transInfo")) {
				if (status == NO_ERROR) {
					Transaction trans = new Transaction(auction, quantity,
							price);
					...
					try {
						...
						log.log(Level.SEVERE,
								"agent could not handle transaction " + trans,
								e);
					}
				} else {
		...
		for (int i = 0; i < transActionsNum; i++) {
			...
			if ((OP_CLEAR_BID & ival) != 0) {
				...
				if (ival == OP_GAME_ENDS) {
					...
					isGameStarted = true;
					d.addTask(currentTime + INFO_UPDATE_PERIOD, "quotes",
							connection, this);
					d.addTask(currentTime + (int) (1.5 * INFO_UPDATE_PERIOD),
							"bids", connection, this);
					if (printOwnDelay > 0) {
						d.addTask(currentTime + printOwnDelay, "printOwn",
								connection, this);
					}
					...
					TimeDispatcher.getDefault().addTask(nextHotelTime,
							"hotelQuotes", connection, this);
					TimeDispatcher.getDefault().addTask(nextFlightTime,
							"flightQuotes", connection, this);
	...
	private void handleQuote(TACMessage msg) {
		...
		while (msg.nextTag()) {
			if (msg.isTag("lastAskPrice")) {
				...
				quote.setAuctionStatus(msg
						.getValueAsInt(Quote.AUCTION_INITIALIZING));
		...
		try {
			...
			log.log(Level.SEVERE, "agent could not handle quoteUpdated for "
					+ quote, e);
		}
	...
	private boolean isLastAuction(Quote quote) {
		...
		if (category == CAT_ENTERTAINMENT) {
			...
			TimeDispatcher.getDefault().addTask(serverTime + 1000, quote,
					connection, this);
			return false;
		} else {
	...
	private void handleBidInfo(TACMessage msg) {
		...
		while (msg.nextTag()) {
			if (msg.isTag("bidString")) {
				...
				rejectReason = Bid.mapRejectReason(msg
						.getValueAsInt(rejectReason));
			} else if (msg.isTag("processingState")) {
				processingState = Bid.mapProcessingState(msg
						.getValueAsInt(processingState));
		...
		// 4. BidInfo is received with same bidHash as 3 but different from
		// msg.Bid
		...
		if (commandStatus != NO_ERROR) {
			...
			if (oldHash == null && !isGameStarted) {
				...
				try {
					...
					log.log(Level.SEVERE, "agent could not handle bidUpdated",
							e);
				}
			}
	...
	private synchronized void clearBid(int transID) {
		...
		while (activeBid != null) {
			if (activeBid.getClearID() == clearID) {
				...
				Bid newBid = new Bid(activeBid, bidString,
						activeBid.getClearHash());
				...
				if (isActiveBid) {
					try {
						...
						log.log(Level.SEVERE,
								"agent could not handle bidUpdated", e);
					}
					if (tableModel != null) {
						tableModel.fireTableRowsUpdated(auction, auction);
					}
	...
	private synchronized void recoverBid(Bid bid) {
		...
		if (bids[auction] != null) {
			log.warning("bid already exist for auction "
					+ getAuctionTypeAsString(auction) + " when recovering bid");
		} else {
			bids[auction] = bid;
		...
		while (msg.nextTag()) {
			if (msg.isTag("auctionIDs")) {
				...
				if (status == GAME_FUTURE) {
					...
					fatalError("could not get auctions for game " + nextGameID
							+ ": status=" + commandStatusToString(status), 5000);
				}
			}
		}
	...
	private void handleGetGame(TACMessage msg) {
		...
		if (gameRunning) {
			TimeDispatcher.getDefault().addTask(startTime + 1000 + gameLength,
					"gameEnds", connection, this);
		}
	}
	...
	private void handleRecoverBidIDs(TACMessage msg) {
		while (msg.nextTag()) {
			if (msg.isTag("auctionBidIDsTuple")) {
				...
				int status = msg.getValueAsInt(NO_ERROR);
				if (status != NO_ERROR) {
					log.severe("could not recover bids for game " + nextGameID
	...
	private void setClient(int client, int arr, int dep, int hotel, int[] events) {
		...
		if ((gameID >= 0) && (startTime > 0)) {
			...
			if (delay < 0) {
				...
				if (delay > 50000) {
					...
					TimeDispatcher.getDefault().addTask(nextGameTime + 1000,
							"gameStarts", connection, this);
				}
			}
		} else if (status != NO_ERROR) {
			fatalError(
					"Failed to get next game"
							+ (gameType != null ? " with game type '"
									+ gameType + '\'' : "") + ": status="
							+ commandStatusToString(status), 5000);
		}
	}
	...
	private class AgentTableModel extends AbstractTableModel {
		...
		public Object getValueAt(int row, int col) {
			...
			case 5:
				Bid bd = bids[row];
				return (bd != null) ? bd.getProcessingStateAsString()
						: "no bid";
			case 6:
				Bid bid = bids[row];
				if (bid != null) {
	...
	public static void main(String[] args) {
		String usage = "Usage: tacagent.jar [-options]\n"
		...
		String agentClass = trim(a.getArgument("-agentimpl",
				config.getProperty("agentimpl", "se.sics.tac.aw.DummyAgent")));
		...
		boolean gui = !(a.hasArgument("-nogui") || "true".equals(config
				.getProperty("nogui", null)));
	...
	private static Properties getConfig(String configFile, File configFP) {
		...
		System.out.println(configFP.getAbsolutePath());

		try {
			if (configFP.exists()) {
				InputStream input = new BufferedInputStream(
						new FileInputStream(configFP));
				try {
