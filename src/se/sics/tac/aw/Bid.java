...
public class Bid {
	...
	private final static String[] rejectName = { "not rejected",
			"self transaction", "buy not allowed", "sell not allowed",
			"price not beat", "active bid changed", "bid not improved",
			"bid not active" };
	private final static int[] rejectCode = { NOT_REJECTED, SELF_TRANSACTION,
			BUY_NOT_ALLOWED, SELL_NOT_ALLOWED, PRICE_NOT_BEAT,
			ACTIVE_BID_CHANGED, BID_NOT_IMPROVED, BID_NOT_ACTIVE };
	...
	private final static String[] stateName = { "unprocessed", "rejected",
			"valid", "withdrawn", "transacted", "replaced", "expired" };
...
//		this.timeSubmitted = timeSubmitted;
	...
	public String getProcessingStateAsString() {
		...
		return (state >= UNPROCESSED) && (state <= stateName.length) ? stateName[state]
