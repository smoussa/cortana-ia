...
public class TACReader extends TACConnection implements Runnable {
	...
	private static final Logger log = Logger.getLogger(TACReader.class
			.getName());
			...
			while (!disconnected && (len = input.read(buffer)) != -1) {
				...
				for (int i = 0; i < len; i++) {
					if (buffer[i] == 0) {
						...
						lastPos = i + 1;
						/*
						 * To test if the server handles agents that does not
						 * read if (msg.indexOf("bidInfo") > 0) {
				...
				if (lastPos < len) {
					lastMessage.append(new String(buffer, lastPos, len
							- lastPos));
				}
			}
		} catch (Throwable e) {
